            ...
            if (tempItem) {
                const tempCart = state.data.map(item => {
                    if (item.id === action.payload.id) {
                        const newQty = item.quantity + (Number(action.payload.quantity) || 0);
                        console.log("Payload:", action.payload);
                        const newTotalPrice = (newQty * Number(item.price)) || 0;
                ...
                const newItem = {
                    ...action.payload,
                    price:parseFloat(action.payload.price) , // Chuyển `price` thành số
                    quantity: Number(action.payload.quantity) || 0, // Chuyển `quantity` thành số
                    totalPrice: (parseFloat(action.payload.price) || 0) * (parseFloat(action.payload.quantity) || 0)
                };
